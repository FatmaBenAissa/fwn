...
function UserTab() {
  ...
  useEffect(()=>{
    ...
    return (
        ...
        <div style={{display:"flex",flexWrap:"wrap",marginTop:-277,justifyContent:"space-around", backgroundColor:'white'}}  >
        ...
        {!users? <h2 style={{marginTop:150}}>  <ReactLoading type={"spokes"} color={"blue"} height={50} width={50} /> </h2> : users.map(el=> 
                      ...
                      <div className="card2" style={{minWidth:219,maxWidth:300,minHeight:200,marginLeft:-30,marginBottom:10}}>
                        <div className="card2-body textcenter" style={{width:263, height:305}}> 
                          <p><img className=" img-fluid" src={el.imageUrl} alt="card image" /></p>
                          <h4 className="card2-title">{el.name} {el.lastName}</h4>
                          <p className="card2-text" style={{marginTop:33, fontSize:14,color:"black"}}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-envelope-fill" viewBox="0 0 16 16">
